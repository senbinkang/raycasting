export class TextureManager {
    constructor() {
        this.size = 64
        this.textures = {}
        this._pixelCache = {}
        this.generateAll()
    }

    _addNoise(canvas, amount = 16) {
        const ctx = canvas.getContext('2d')
        const img = ctx.getImageData(0, 0, canvas.width, canvas.height)
        const d = img.data
        for (let i = 0; i < d.length; i += 4) {
            const n = (Math.random() - 0.5) * amount
            d[i] = Math.max(0, Math.min(255, d[i] + n))
            d[i + 1] = Math.max(0, Math.min(255, d[i + 1] + n))
            d[i + 2] = Math.max(0, Math.min(255, d[i + 2] + n))
        }
        ctx.putImageData(img, 0, 0)
    }

    _newCanvas() {
        const c = document.createElement('canvas')
        c.width = this.size
        c.height = this.size
        c.getContext('2d', { willReadFrequently: true })
        return c
    }

    generateBrick(baseR, baseG, baseB) {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')

        ctx.fillStyle = `rgb(${baseR},${baseG},${baseB})`
        ctx.fillRect(0, 0, size, size)

        const seamR = Math.floor(baseR * 0.4)
        const seamG = Math.floor(baseG * 0.4)
        const seamB = Math.floor(baseB * 0.4)
        ctx.fillStyle = `rgb(${seamR},${seamG},${seamB})`

        const brickH = 8,
            brickW = 16
        for (let row = 0; row < size / brickH; row++) {
            const offset = (row % 2) * (brickW / 2)
            ctx.fillRect(0, row * brickH, size, 2)
            for (let col = -1; col < size / brickW + 1; col++) {
                const bx = col * brickW + offset
                ctx.fillRect(bx, row * brickH, 2, brickH)
            }
        }
        this._addNoise(c, 12)
        return c
    }

    generateStone(baseR, baseG, baseB) {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.fillStyle = `rgb(${baseR},${baseG},${baseB})`
        ctx.fillRect(0, 0, size, size)

        for (let i = 0; i < 60; i++) {
            const x = Math.random() * size
            const y = Math.random() * size
            const r = 4 + Math.random() * 8
            const shade = 0.65 + Math.random() * 0.4
            const rr = Math.floor(baseR * shade)
            const gg = Math.floor(baseG * shade)
            const bb = Math.floor(baseB * shade)
            ctx.fillStyle = `rgba(${rr},${gg},${bb},0.9)`
            ctx.beginPath()
            ctx.arc(x, y, r, 0, Math.PI * 2)
            ctx.fill()
        }
        ctx.strokeStyle = `rgba(${Math.floor(baseR * 0.4)},${Math.floor(baseG * 0.4)},${Math.floor(baseB * 0.4)},0.8)`
        ctx.lineWidth = 1
        for (let i = 0; i < 8; i++) {
            ctx.beginPath()
            const sx = Math.random() * size
            const sy = Math.random() * size
            const ex = sx + (Math.random() - 0.5) * 20
            const ey = sy + (Math.random() - 0.5) * 20
            ctx.moveTo(sx, sy)
            ctx.lineTo(ex, ey)
            ctx.stroke()
        }
        this._addNoise(c, 20)
        return c
    }

    generateWood(baseR, baseG, baseB) {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')

        ctx.fillStyle = `rgb(${baseR},${baseG},${baseB})`
        ctx.fillRect(0, 0, size, size)

        for (let i = 0; i < size; i += 16) {
            const shade = 0.7 + Math.random() * 0.5
            const rr = Math.floor(baseR * shade)
            const gg = Math.floor(baseG * shade)
            const bb = Math.floor(baseB * shade)
            ctx.fillStyle = `rgb(${rr},${gg},${bb})`
            ctx.fillRect(i, 0, 14, size)
            ctx.fillStyle = `rgb(${Math.floor(baseR * 0.4)},${Math.floor(baseG * 0.4)},${Math.floor(baseB * 0.4)})`
            ctx.fillRect(i + 14, 0, 2, size)
        }
        for (let y = 0; y < size; y += 4) {
            const a = 0.03 + Math.random() * 0.05
            ctx.fillStyle = `rgba(0,0,0,${a})`
            ctx.fillRect(0, y, size, 1)
        }
        this._addNoise(c, 12)
        return c
    }

    generateFloorChecker() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        const t = size / 2
        for (let y = 0; y < 2; y++) {
            for (let x = 0; x < 2; x++) {
                if ((x + y) % 2 === 0) {
                    ctx.fillStyle = 'rgb(70,70,75)'
                } else {
                    ctx.fillStyle = 'rgb(50,50,55)'
                }
                ctx.fillRect(x * t, y * t, t, t)
            }
        }
        ctx.strokeStyle = 'rgb(30,30,30)'
        ctx.lineWidth = 1
        ctx.strokeRect(0, 0, size, size)
        this._addNoise(c, 10)
        return c
    }

    generateCeil() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        const t = size / 2
        for (let y = 0; y < 2; y++) {
            for (let x = 0; x < 2; x++) {
                if ((x + y) % 2 === 0) {
                    ctx.fillStyle = 'rgb(105,105,115)'
                } else {
                    ctx.fillStyle = 'rgb(90,90,100)'
                }
                ctx.fillRect(x * t, y * t, t, t)
            }
        }
        ctx.strokeStyle = 'rgb(60,60,70)'
        ctx.lineWidth = 1
        ctx.strokeRect(0, 0, size, size)
        this._addNoise(c, 8)
        return c
    }

    generateEnemy() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)

        ctx.fillStyle = 'rgb(210, 50, 50)'
        ctx.beginPath()
        ctx.arc(size / 2, size / 2, size * 0.38, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgba(120,0,0,0.55)'
        ctx.beginPath()
        ctx.arc(size / 2, size * 0.62, size * 0.38, 0, Math.PI)
        ctx.fill()

        ctx.fillStyle = 'rgb(255,255,255)'
        ctx.beginPath()
        ctx.arc(size * 0.35, size * 0.4, size * 0.08, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.65, size * 0.4, size * 0.08, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.beginPath()
        ctx.arc(size * 0.35, size * 0.4, size * 0.04, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.65, size * 0.4, size * 0.04, 0, Math.PI * 2)
        ctx.fill()

        ctx.strokeStyle = 'rgb(0,0,0)'
        ctx.lineWidth = 2
        ctx.beginPath()
        ctx.moveTo(size * 0.3, size * 0.62)
        for (let i = 0; i <= 10; i++) {
            const t = i / 10
            const x = size * 0.3 + t * size * 0.4
            const y = size * 0.62 + (i % 2 === 0 ? 0 : size * 0.05)
            ctx.lineTo(x, y)
        }
        ctx.stroke()

        this._addNoise(c, 10)
        return c
    }

    generateFastEnemy() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)
        ctx.fillStyle = 'rgb(230, 140, 40)'
        ctx.beginPath()
        ctx.moveTo(size * 0.5, size * 0.15)
        ctx.lineTo(size * 0.85, size * 0.8)
        ctx.lineTo(size * 0.15, size * 0.8)
        ctx.fill()
        ctx.fillStyle = 'rgb(255,255,255)'
        ctx.beginPath()
        ctx.arc(size * 0.4, size * 0.5, size * 0.07, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.6, size * 0.5, size * 0.07, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.beginPath()
        ctx.arc(size * 0.4, size * 0.5, size * 0.035, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.6, size * 0.5, size * 0.035, 0, Math.PI * 2)
        ctx.fill()
        ctx.strokeStyle = 'rgb(0,0,0)'
        ctx.lineWidth = 2
        ctx.beginPath()
        ctx.moveTo(size * 0.32, size * 0.68)
        ctx.lineTo(size * 0.5, size * 0.58)
        ctx.lineTo(size * 0.68, size * 0.68)
        ctx.stroke()
        this._addNoise(c, 10)
        return c
    }

    generateTankEnemy() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)
        ctx.fillStyle = 'rgb(140, 50, 200)'
        ctx.fillRect(size * 0.15, size * 0.15, size * 0.7, size * 0.7)
        ctx.fillStyle = 'rgb(100,30,150)'
        ctx.fillRect(size * 0.15, size * 0.5, size * 0.7, size * 0.35)
        ctx.fillStyle = 'rgb(255,255,255)'
        ctx.beginPath()
        ctx.arc(size * 0.35, size * 0.38, size * 0.08, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.65, size * 0.38, size * 0.08, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.beginPath()
        ctx.arc(size * 0.35, size * 0.38, size * 0.04, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.65, size * 0.38, size * 0.04, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.fillRect(size * 0.35, size * 0.6, size * 0.3, size * 0.08)
        this._addNoise(c, 10)
        return c
    }

    generateGhostEnemy() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)
        ctx.fillStyle = 'rgb(50, 200, 200)'
        ctx.beginPath()
        ctx.ellipse(size / 2, size / 2, size * 0.35, size * 0.42, 0, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgba(20,120,120,0.5)'
        ctx.beginPath()
        ctx.ellipse(size / 2, size * 0.6, size * 0.35, size * 0.3, 0, 0, Math.PI)
        ctx.fill()
        ctx.fillStyle = 'rgb(255,255,255)'
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.4, size * 0.06, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.62, size * 0.4, size * 0.06, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.4, size * 0.03, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.62, size * 0.4, size * 0.03, 0, Math.PI * 2)
        ctx.fill()
        this._addNoise(c, 10)
        return c
    }

    generateRangedEnemy() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)
        ctx.fillStyle = 'rgb(60, 180, 80)'
        ctx.beginPath()
        ctx.arc(size / 2, size / 2, size * 0.35, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(60,60,60)'
        ctx.fillRect(size * 0.55, size * 0.45, size * 0.3, size * 0.1)
        ctx.fillStyle = 'rgb(30,30,30)'
        ctx.fillRect(size * 0.82, size * 0.43, size * 0.08, size * 0.14)
        ctx.fillStyle = 'rgb(255,255,255)'
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.38, size * 0.07, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.58, size * 0.07, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = 'rgb(0,0,0)'
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.38, size * 0.035, 0, Math.PI * 2)
        ctx.fill()
        ctx.beginPath()
        ctx.arc(size * 0.38, size * 0.58, size * 0.035, 0, Math.PI * 2)
        ctx.fill()
        this._addNoise(c, 10)
        return c
    }

    generateItem() {
        const size = this.size
        const c = this._newCanvas()
        const ctx = c.getContext('2d')
        ctx.clearRect(0, 0, size, size)

        ctx.fillStyle = 'rgb(235,235,235)'
        ctx.fillRect(size * 0.2, size * 0.2, size * 0.6, size * 0.6)

        ctx.fillStyle = 'rgb(200,30,30)'
        ctx.fillRect(size * 0.45, size * 0.25, size * 0.1, size * 0.5)
        ctx.fillRect(size * 0.25, size * 0.45, size * 0.5, size * 0.1)

        ctx.strokeStyle = 'rgb(80,80,80)'
        ctx.lineWidth = 2
        ctx.strokeRect(size * 0.2, size * 0.2, size * 0.6, size * 0.6)

        this._addNoise(c, 8)
        return c
    }

    generateAll() {
        this.textures[1] = this.generateBrick(200, 70, 70)
        this.textures[2] = this.generateBrick(70, 120, 210)
        this.textures[3] = this.generateStone(150, 150, 160)
        this.textures[4] = this.generateWood(180, 130, 70)

        this.textures[10] = this.generateFloorChecker()
        this.textures[11] = this.generateCeil()

        this.textures[201] = this.generateEnemy()
        this.textures[202] = this.generateItem()
        this.textures[203] = this.generateFastEnemy()
        this.textures[204] = this.generateTankEnemy()
        this.textures[205] = this.generateGhostEnemy()
        this.textures[206] = this.generateRangedEnemy()
    }

    getTextureColumn(textureIndex, texX) {
        const cacheKey = textureIndex + '_' + texX
        if (this._colCache && this._colCache[cacheKey]) {return this._colCache[cacheKey]}

        const canvas = this.textures[textureIndex]
        if (!canvas) {return null}
        const ctx = canvas.getContext('2d')
        const img = ctx.getImageData(texX, 0, 1, this.size)
        const d = img.data
        const out = new Array(this.size)
        for (let y = 0; y < this.size; y++) {
            const i = y * 4
            out[y] = [d[i], d[i + 1], d[i + 2]]
        }

        if (!this._colCache) {this._colCache = {}}
        this._colCache[cacheKey] = out
        return out
    }

    getPixels(textureIndex) {
        if (this._pixelCache[textureIndex]) {return this._pixelCache[textureIndex]}
        const canvas = this.textures[textureIndex]
        if (!canvas) {return null}
        const ctx = canvas.getContext('2d')
        const img = ctx.getImageData(0, 0, this.size, this.size)
        this._pixelCache[textureIndex] = img
        return img
    }
}
